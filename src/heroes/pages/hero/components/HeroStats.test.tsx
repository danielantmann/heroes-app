...
describe("HeroStats", () => {
  ...
  test("should change the percentage when a hero is added to favorites", () => {
    localStorage.setItem("favorites", JSON.stringify([mockHero]));
    renderHeroStats(mockSummaryData);

    const favoriteElementPercentage = screen.getByTestId("favorite-percentage");
    expect(favoriteElementPercentage.innerHTML).toContain("4.00%");
    const favoriteElementCount = screen.getByTestId("favorite-count");
    expect(favoriteElementCount.innerHTML).toContain("1");
  });
});
